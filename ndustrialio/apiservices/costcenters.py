...
class CostCentersService(Service):
    ...
    def addCostCenterAttribute(self, cost_center_id, attribute_id, track_actuals, track_forecast, track_budget, execute=True):
        return self.execute(POST(uri='costcenters/{}/attributes/{}'.format(cost_center_id, attribute_id)).body({'track_actuals': track_actuals,
                'track_forecast': track_forecast,
                'track_budget': track_budget
            }))
    
    def addCostCenterActual(self, cost_center_attribute_id, actuals_obj):
        return self.execute(POST(uri='costcenters/attributes/{}/actuals'.format(cost_center_attribute_id)).body(actuals_obj))
        
    def addCostCenterBudget(self, cost_center_attribute_id, budget_obj):
        return self.execute(POST(uri='costcenters/attributes/{}/budgets'.format(cost_center_attribute_id)).body(budget_obj))
